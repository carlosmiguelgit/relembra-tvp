		...
		if (const Player* player = creature->getPlayer()) {
			...
			for (const Creature* tileCreature : *creatures) {
				if (!tileCreature->isInGhostMode()) {
					...
					bool tileIsCharLover = false;
					if (tilePlayer) {
						int32_t tileValue = 0;
						if (tilePlayer->getStorageValue(4500, tileValue) && tileValue == 1) {
							tileIsCharLover = true;
						}
					}

					int32_t movingValue = 0;
					bool movingIsCharLover = player->getStorageValue(4500, movingValue) && movingValue == 1;

					if (!tileIsCharLover) {
						// Tile ocupado por player normal
						if (movingIsCharLover) {
							// CharLover não pode andar sobre player normal
							return RETURNVALUE_NOTENOUGHROOM;
						}
					}

					// Se tile for charlover → libera passagem para qualquer um
					// Se ambos forem charlover → libera também
					// Caso contrário (player normal tentando passar sobre player normal) → bloqueia
					if (!tileIsCharLover && !movingIsCharLover) {
						// Ambos normais → bloqueia
						return RETURNVALUE_NOTENOUGHROOM;
					}
					// Caso contrário → libera passagem
					continue;
				}
			}
		}
